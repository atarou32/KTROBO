...
float4x4 MyMatrixMultiply(float4x4 a, float4x4 b) {
...
float4x4 MyMatrixInverse(float4x4 mat) {
...
float getValueFromTexColor(float4 color) {
    float ans = 0;
    float code = 0.1 * 1/256.0f;

    uint r = floor((color.x + code)*256);
    uint g = floor((color.y + code)*256);
    uint b = floor((color.z + code)*256);
    uint a = floor((color.w + code)*256);
...
float4x4 getMatrixFromTex(Texture2D tex, float2 texcoord, int tex_width, int tex_height) {
float4x4 ans;
...
float4x4 getMatrixFromVecF(float4 vo, float4 vt, float4 vthr, float4 vf) {
float4x4 ans;
ans._11 = vo.x;
ans._12 = vo.y;
ans._13 = vo.z;
...
float4x4 getZeroKAKUDAI(float4x4 a) {
float4x4 ans = a;
...
float4x4 getCombinedMatrix(uint instance_index, uint bone_index) {
float4x4 ans;
...
float4x4 getLocalMatrix(uint skeleton_index, uint bone_index) {
float4x4 ans;
uint texel_per_bone = 16;
...
float4x4 getOffsetMatrix(uint anime_first_index, 
...
float2 tex_coord = float2(x*xoffset, y*yoffset);
float4x4 anime_first_matrix = getMatrixFromTex(basisTex, tex_coord, KTROBO_SHADER_VALUE_BASISTEX_WIDTH_HEIGHT, KTROBO_SHADER_VALUE_BASISTEX_WIDTH_HEIGHT);
...
float4x4 anime_last_matrix = getMatrixFromTex(basisTex, tex_coord,  KTROBO_SHADER_VALUE_BASISTEX_WIDTH_HEIGHT, KTROBO_SHADER_VALUE_BASISTEX_WIDTH_HEIGHT);
...
float4x4 getCombinedMatrix(uint place) {
...
float4x4 kakeru_matrix = getMatrixFromVecF(kakeruMONE, kakeruMTWO, kakeruMTHREE, kakeruMFOUR);
...
if ((parent_bone_index != KTROBO_MESH_BONE_NULL) && (is_root_bone ==0)) {
    float4x4 par_rest;
    float4x4 temp_matrix;
    float4x4 b_matrix_local = getLocalMatrix(skeleton_index, bone_index);
    float4x4 pb_combined_matrix = getCombinedMatrix(parent_instance_index, parent_bone_index);
    float4x4 b_offset_matrix = getOffsetMatrix(anime_first_index, anime_last_index, anime_first_weight, skeleton_index, bone_index);
    temp_matrix = MyMatrixMultiply(b_matrix_local, pb_combined_matrix);
    par_rest = MyMatrixInverse(temp_matrix);
    ans_cm = MyMatrixMultiply(pb_combined_matrix, 
                            MyMatrixMultiply(par_rest, 
                              MyMatrixMultiply(b_offset_matrix,temp_matrix)));
}

if (parent_bone_index == KTROBO_MESH_BONE_NULL) {
    float4x4 b_matrix_local = getLocalMatrix(skeleton_index, bone_index);
    float4x4 mat_inv = MyMatrixInverse(b_matrix_local);
    float4x4 b_offset_matrix = getOffsetMatrix(anime_first_index, anime_last_index, anime_first_weight, skeleton_index, bone_index);
    ans_cm = MyMatrixMultiply(mat_inv, MyMatrixMultiply(b_offset_matrix, b_matrix_local));
}
...
if ((parent_bone_index != KTROBO_MESH_BONE_NULL) && is_root_bone > 0 && connect_without_matrix_local >0) {
    ...
    float4x4 par_rest;
    float4x4 temp_matrix;
    float4x4 bp_combined_matrix = getCombinedMatrix(parent_instance_index, parent_bone_index);
    float4x4 b_offset_matrix = getOffsetMatrix(anime_first_index, anime_last_index, anime_first_weight, skeleton_index, bone_index);
    float4x4 tem = MyMatrixMultiply(b_matrix_local, kakeru_matrix);
...
if ((parent_bone_index != KTROBO_MESH_BONE_NULL) && is_root_bone > 0 && connect_without_matrix_local ==0) {
    float4x4 b_matrix_local = getLocalMatrix(skeleton_index, bone_index);
    float4x4 pb_matrix_local = getLocalMatrix(parent_skeleton_index, parent_bone_index);
    float4x4 pb_combined_matrix = getCombinedMatrix(parent_instance_index, parent_bone_index);
    float4x4 b_offset_matrix = getOffsetMatrix(anime_first_index, anime_last_index, anime_first_weight, skeleton_index, bone_index);
    float4x4 tem = MyMatrixMultiply(b_matrix_local, kakeru_matrix);
    float4x4 p = MyMatrixInverse(tem);
...
[numthreads(32,1,1)]
void CalcCS(uint3 gtid: SV_GroupThreadID, uint3 gid: SV_GroupID )
{
//uint iid = gtid.x;
//uint b_index = gid.x;
uint struct_idx = gid.x * 32 + gtid.x;
...
float4x4 cb = getCombinedMatrix(place);
float2 indexs = getIndexs(place);

float4 cb_one = float4(cb._11,cb._12,cb._13,cb._14);
float4 cb_two = float4(cb._21,cb._22,cb._23,cb._24);
float4 cb_three = float4(cb._31, cb._32, cb._33, cb._34);
float4 cb_four = float4(cb._41, cb._42,cb._43, cb._44);
uint index = struct_idx * 20 * 4;
uint instance_id = indexs.x;
uint bone_index = indexs.y;
// ŒvŽZŒ‹‰Ê‚ðƒoƒt‚ÉŠi”[‚·‚é
ansBuf.Store4(index + 16, asuint(cb_one));
ansBuf.Store4(index + 16 + 16 * 1, asuint(cb_two));
ansBuf.Store4(index + 16 + 16 * 2, asuint(cb_three));
ansBuf.Store4(index + 16 + 16 * 3, asuint(cb_four));
