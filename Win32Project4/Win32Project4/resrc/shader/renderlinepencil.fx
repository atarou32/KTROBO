...
GSPSInput inp;
inp.MyInfo = input[0].MyInfo;//int2(0,0);
inp.MyInfo2 = input[0].MyInfo2;//uint2(0,0);
inp.Pos = uint2(0,0);
inp.TexCoord = float2(0.0f,0.0f);
inp.offset = input[0].offset;
float W = screen_x;
float H = screen_y;
...
int nwidthidx = 1;//(input[0].MyInfo2.x >> 4);
int widthidx = 1;//input[0].MyInfo2.x - (nwidthidx << 4);
int pen_index = 1;//input[0].MyInfo2.y;
float width = zoom*2;//pen_width[pen_index][widthidx%4][widthidx/4];
float nwidth = zoom*2;//pen_width[pen_index][nwidthidx%4][nwidthidx/4];
...
float4 PSFunc(GSPSInput input ) : SV_Target {
 ...
 return float4(input.MyInfo2.x/255.0f,input.MyInfo2.y/255.0f,input.offset.x/255.0f,input.offset.y/255.0f);
}