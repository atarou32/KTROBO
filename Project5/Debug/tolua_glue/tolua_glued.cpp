...
/*
void KTROBO::YARITORI_getMYRECT(lua_State* L, MYRECT* class_inst, int index) {
...
void KTROBO::YARITORI_setMYRECT(lua_State* L, MYRECT* class_inst, int index) {
	char temp[1024];
	memset(temp,0,1024);
	sprintf_s(temp, 1024, "%f", class_inst->left);
	lua_pushnumber(L, 1);
	lua_pushstring(L, temp);
	lua_settable(L, index+3);

	memset(temp,0,1024);
	sprintf_s(temp, 1024, "%f", class_inst->right);
	lua_pushnumber(L, 2);
	lua_pushstring(L, temp);
	lua_settable(L, index+3);

	memset(temp,0,1024);
	sprintf_s(temp, 1024, "%f", class_inst->top);
	lua_pushnumber(L, 3);
	lua_pushstring(L, temp);
	lua_settable(L, index+3);

	memset(temp,0,1024);
	sprintf_s(temp, 1024, "%f", class_inst->bottom);
	lua_pushnumber(L, 4);
	lua_pushstring(L, temp);
	lua_settable(L, index+3);
}
...
void KTROBO::YARITORI_setKAERITIMYRECT(lua_State* L, MYRECT* class_inst) {
lua_newtable(L);
int top = lua_gettop(L);
YARITORI_setMYRECT(L, class_inst, top);
}
*/
